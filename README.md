...
- **批量投递**：最多保留 20 个待处理岗位，每个岗位保存 1 条可编辑招呼语，并绑定生成时使用的简历
...
4. **单岗位分析**：点击"分析当前岗位"，查看 JD 匹配结果，在 3 种风格中选择或编辑后确认发送
5. **批量生成**：在投递清单点击"批量生成招呼语"，每个岗位生成 1 条可直接编辑的招呼语
6. **开始投递**：确认岗位、招呼语和绑定简历后点击"开始投递"；插件逐项打开详情、核验岗位并发送
7. **查看记录**：成功投递的岗位记入岗位库，可导出 CSV
